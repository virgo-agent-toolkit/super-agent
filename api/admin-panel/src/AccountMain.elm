...
import Account exposing(Uuid, NewAccount, Row, Query, Columns, Rows, Stats, Results)
...
update action model =
  case action of
    Name value -> let model = {model | name = value} in
      (model, doQuery model)
    Limit value ->
      case toInt value of
    ...
    Delete id ->
      (model, do Account.delete id Changed)
    Save ->
      ({model | current = Nothing}, Effects.none)
    Changed (Ok True) ->
      (model, doQuery model)
...
renderRow address current (id, name) =
  tr [key id] (
    td [] [ text id ] ::
    if
      isSelected current id
...
doQuery model = do Account.query (extractQuery model) QueryResults
